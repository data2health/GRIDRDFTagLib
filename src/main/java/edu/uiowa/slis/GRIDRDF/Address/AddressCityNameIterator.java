...
public class AddressCityNameIterator extends edu.uiowa.slis.GRIDRDF.TagLibSupport {
	static AddressCityNameIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(AddressCityNameIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://www.grid.ac/ontology/cityName> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				cityName = sol.get("?s").toString();
				firstInstance = true;
